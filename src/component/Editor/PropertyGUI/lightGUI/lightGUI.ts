...
export const lightGUI = (light: Light, folder: GUI) => {
  ...
  positionFolder.add(light.position, "x", -minP, minP, 0.01).name("x");
  // positionFolder.add(light.position, "y", -minP, minP, 0.01).name("y");
  // positionFolder.add(light.position, "z", -minP, minP, 0.01).name("z");
};
